...
def save_memory(memory):
    try:
        with open(MEMORY_FILE, "w") as f:
            json.dump(memory, f)
    except: print("Hafıza yazılamadı!")
...
# --- STRATEJİ 1: HAFTALIK DÜŞEN KIRILIMI ---
def check_düşen_haftalik(df):
    if len(df) < 40: return None
    highs = df['High'].values
    closes = df['Close'].values
    
    # Haftalıkta gürültüyü azaltmak için window=5
    window = 5
    pivots = []
    ...
    # Haftalık kapanış düşen trend çizgisinin üzerindeyse
    if closes[curr_idx] > cizgi_bugun and closes[curr_idx-1] <= cizgi_gecen_hafta:
        return round(closes[curr_idx], 2)
    return None

# --- STRATEJİ 2: HAFTALIK TURTLE (20 HAFTALIK EN YÜKSEK) ---
def check_turtle_haftalik(df):
    if len(df) < 22: return None
    # Son 20 haftanın zirvesi (içinde bulunulan hafta hariç)
...
def main():
    ...
    print("Haftalık Macro Analiz Başlıyor (5 Yıllık Veri)...")
    
    for ticker in TICKERS:
        try:
            # interval="1wk" ve period="5y" haftalık analiz için şart
            ...
            if fiyat_d and memory.get(f"{ticker}_D") != "OK":
                mesajlar.append(f"📉 *{ticker}* Haftalık Düşen Kırılım! (Fiyat: {fiyat_d})")
                memory[f"{ticker}_D"] = "OK"

            # 2. Turtle
            fiyat_t = check_turtle_haftalik(df)
            if fiyat_t and memory.get(f"{ticker}_T") != "OK":
                mesajlar.append(f"🐢 *{ticker}* 20 Haftalık Turtle Kırılım! (Zirve: {fiyat_t})")
                memory[f"{ticker}_T"] = "OK"
                
        except: continue

    if mesajlar:
        full_msg = "🗓️ *HAFTALIK MACRO TARAMA RAPORU* 🗓️\n\n" + "\n".join(mesajlar)
        send_telegram_message(full_msg)
        save_memory(memory)
        print("Haftalık rapor gönderildi.")
    else:
        print("Yeni haftalık kırılım yok.")
